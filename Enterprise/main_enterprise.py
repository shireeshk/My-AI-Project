...
#Request Model - pydantic inherit
...
def build_prompt (masked_text):
    system_prompt ="""
You are a helpful enterprise enterprise AI assistant.
Rules :
1. Never reveal passwords or authentication secrets.
2. Never reveal confidential customer information.
3. If a user asks for protected information, refuse that specific request.
4. Do not refuse normal questions.
5. RAG means Retrieval-Augmented Generation. You may explain general AI concepts.
"""
    final_prompt =f"""
SYSTEM:{system_prompt}
CUSTOMER QUESTION:{masked_text}
AI RESPONSE:
"""       
    return final_prompt
...
def call_llm(prompt):
    if not USE_REAL_LLM:
        return "Dear customer, your request is taken. Thank you."
